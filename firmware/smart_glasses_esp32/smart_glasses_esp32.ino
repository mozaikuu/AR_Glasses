...
std::function<void(float, float, float)> onGestureDetected = nullptr;
...
void loopIMU() {
  ...
  if (millis() - lastRead > 10) {  // 100Hz
    ...
    if (intensity > 2.0) {  // Motion detected
...
String detectSimpleGesture() {
  ...
  if (accelY > 8.0) return "tilt_forward";
  if (accelY < -8.0) return "tilt_backward";
  if (accelX > 8.0) return "tilt_right";
  if (accelX < -8.0) return "tilt_left";
...
void onGesture(std::function<void(float, float, float)> callback) {
